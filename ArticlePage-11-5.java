...
import java.io.File;   //TakesScreenshot
...
public class ArticlePage extends TribunePage {
    ...
    private WebElement twitterIconElement;
    ...
    private WebElement googleplusIconElement;
    ...
    private WebElement linkedinIconElement;
     ...
     * @throws InterruptedException
     */
    private void clickAndVerifyGooglePlusPopup() throws InterruptedException {
        String parentWindowHandle = TribuneDriver.getInstance().getDriver().getWindowHandle();
        verifyPlusURL();
        TribuneDriver.getInstance().getDriver().switchTo().window(parentWindowHandle);
    }
     ...
     * @throws InterruptedException
     */
    private void clickAndVerifyLinkedInPopup() throws InterruptedException {
        String parentWindowHandle = TribuneDriver.getInstance().getDriver().getWindowHandle();
        verifyLinkedinURL();
        TribuneDriver.getInstance().getDriver().switchTo().window(parentWindowHandle);
    }
     ...
     * @throws InterruptedException
     */
    private void clickAndVerifyStumbleUponPopup() throws InterruptedException {
        String parentWindowHandle = TribuneDriver.getInstance().getDriver().getWindowHandle();
        verifyStumbleText();
        TribuneDriver.getInstance().getDriver().switchTo().window(parentWindowHandle);
    }
     ...
     * @throws InterruptedException
     */
    private void clickAndVerifyDiggPopup() throws InterruptedException {
        String parentWindowHandle = TribuneDriver.getInstance().getDriver().getWindowHandle();
        verifyDiggText();
        TribuneDriver.getInstance().getDriver().switchTo().window(parentWindowHandle);
    }
     ...
     * @throws InterruptedException
     */
    void clickAndVerifyEmailPopup() throws InterruptedException {
        String parentWindowHandle = TribuneDriver.getInstance().getDriver().getWindowHandle();
        verifyEmailText();
        TribuneDriver.getInstance().getDriver().switchTo().window(parentWindowHandle);
    }
    ...
    /**
     * This is a method for helping to switch between two windows
     * @return WebDriver
     */
    public WebDriver switchHandleToPopup() {
        // save the current window handle.
        WebDriver popup = null;
        Iterator<String> windowIterator = TribuneDriver.getInstance().getDriver().getWindowHandles().iterator();
        while(windowIterator.hasNext()) {
            String windowHandle = windowIterator.next();
            System.out.println(windowHandle);
            popup = TribuneDriver.getInstance().getDriver().switchTo().window(windowHandle);
        }
        return popup;
    }
    ...
    private int checkTopAd() {
        ...
        if (topAd.size() > 0) {
            ...
            Utils.logWrongCheck("Top Ad (leaderboard) is missing in the page.");
            errors++;
        }
        return errors;
    }
    ...
    public boolean verifyShareQuoteONTwitter(String shareSocialXpath,String twitterXpath, String articleTitle,String quote) throws InterruptedException{
        boolean flag = false;
        String currentUrl = getCurrentUrl();
        String parentHandler = driver.getWindowHandle();
        String windowtitle = getTitle();
        ...
        flag =	gigyaPage.verifySharingPullQuoteOnTwitter(currentUrl, articleTitle, "facebooksharelinePQuote", windowtitle,quote,handler);
        
        return flag;
    }
    
    //*************************************BEGIN COMMENTS NGUX-131 CODE***********************************************************************
    ...
    public void verifyCommentsleaderboard(String url) throws InterruptedException, IOException {
        goToUrl(url);
        Utils.writeToLog("ENTERING VERIFYCOMMENTSLEADERBOARD METHOD");
        //        boolean visible = true;
        try {						
            //			Assert.assertTrue(findElementByXpath("//*[@data-state='adloader_loaded'][@data-adloader-position='commentsPanel_headerLeaderboard']").isDisplayed());
            //			Assert.assertTrue(isleaderboardVisible(), "leaderboard is not visible");
            Assert.assertNotNull(Commentsleaderboard);
            if(Commentsleaderboard.isDisplayed()) {	
                Utils.logCorrectCheck("Leaderboard is displayed on page.");
            } else {
                Utils.logWrongCheck("Leaderboard is missing on page.");
            }
            Utils.writeToLog("CORRECT: assert on leaderboard element");
            Utils.writeToLog("CORRECT: leaderboard on the Comments page.");
        } catch (NoSuchElementException e) {
            Utils.writeToLog("Commentsleaderboard problem");
        }
    }
    ...
    //xpath = /html/body/div[3]/div[2]/div/section/div/header/span/header/span
    //css = html body div.trb_allContentWrapper div.trb_article_panelWrapper div section div.trb_panelmod_openPanel header span.trb_panelmod_openPanel_title header.trb_mastheadsmall span.trb_masthead_subtitle
    @FindBy(xpath="/html/body/div[3]/div[2]/div/section/div/header/span/header/span")   //"COMMENTS"
    private WebElement CommentsMasthead;
    public void verifyCommentsMasthead(String url) throws InterruptedException, IOException {
        goToUrl(url);
        Utils.writeToLog("ENTERING VERIFYCOMMENTSMASTHEAD METHOD");
        //check for selenium webelement create exception
        if(CommentsMasthead.isDisplayed()) {
            Utils.logCorrectCheck("Masthead is displayed on page.");
        } else {
            Utils.logWrongCheck("Masthead is missing on page.");
        }
        try {
            //        	Assert.assertTrue(findElementByXpath("//header[@class='trb_mastheadsmall']").isDisplayed());
            Assert.assertTrue(isMastheadVisible(), "masthead is not visible");
            Assert.assertNotNull(CommentsMasthead);
            Utils.writeToLog("CORRECT: assert on masthead element");
            Utils.writeToLog("CORRECT: masthead on the Comments page");
        } catch (NoSuchElementException e) {
            Utils.writeToLog("screenshot problem");
        }
    }
    ...
    @FindBy(xpath="//a[@data-reg-handler='signInHandler']")
    private WebElement signInLink;
    @FindBy(css = "html body div#reg-overlay div.reg-dialog.login div.reg-content form button.reg-submit")
    private WebElement submitLogInButton;
    public void signIntoCommentsAccount(String url) throws InterruptedException, IOException {
        Utils.writeToLog("url to load: " + url);
        goToUrl(url);
        Utils.writeToLog("ENTERING VERIFYCOMMENTSPANEL METHOD->LOG IN");
        signInLink.click(); 
        Assert.assertTrue(isLogInButtonVisible(), "submit button is not visible");
        WebDriver newwindow = switchHandleToPopup();
        newwindow.findElement(By.name("email")).clear();
        newwindow.findElement(By.name("email")).sendKeys("scottmaretick@tribune.com");
        newwindow.findElement(By.name("password")).clear();
        newwindow.findElement(By.name("password")).sendKeys("sm110751");
        submitLogInButton.click();
        //        TribuneDriver.getInstance().getDriver().findElement(By.xpath("/html/body/div[8]/div/div[2]/form/button")).click();  //-- changes div[x+1]
        //css = html body div#reg-overlay div.reg-dialog.login div.reg-content form button.reg-submit
        //        TribuneDriver.getInstance().getDriver().findElement(By.cssSelector("html body div#reg-overlay div.reg-dialog.login div.reg-content form button.reg-submit")).click();
        Utils.writeToLog("CORRECT: Log on to Comment Page successfully.");
        Utils.writeToLog("ENTERING VERIFYCOMMENTSPANEL METHOD->WRITE COMMENT");
        Date date = new Date();
        //		TribuneDriver.getInstance().getDriver().findElement(By.cssSelector("html body div#reg-overlay div.reg-dialog.login div.reg-content form button.reg-submit")).click();
        //		TribuneDriver.getInstance().getDriver().findElement(By.xpath("//form[@class='vf-comment-form']//textarea")).sendKeys("this is a test comment on " + date);
        //		/html/body/div[3]/div[2]/div/section/div/section/div/div/div/div[3]/form/ul/li[5]/div/button = xpath Submit button
        //		 css = html body div.trb_allContentWrapper div.trb_article_panelWrapper div section div.trb_panelmod_openPanel section.trb_panelmod_openPanel_mainContents div.viafoura div.vf-comments.vf-widget div.vf-commenting div.vf-comment-box form.vf-comment-form ul.vf-comment-controls.vf-clearfix li.vf-right div.vf-comment-submit-wrap button.vf-comment-submit		
        //		try {
        //			Assert.assertNotNull(submitLogInButton);	//FUNCTIONALITY NOT WORKING 10/28
        //			Assert.assertTrue(isLogInButtonVisible(), "comments submit button is not visible");
        //			Utils.writeToLog("CORRECT: Log Out link on Comment Page link is present.");
        //		} catch (NoSuchElementException e2) {
        //			Utils.writeToLog("CommentsLogInlink problem");
        //		}
        
        //	THIS IS TO LOG OUT OF COMMENTS PAGE
        //	public void signOutCommentsAccount(String url) throws InterruptedException, IOException {
        Utils.writeToLog("ENTERING VERIFYCOMMENTSPANEL METHOD->LOG OUT");
        WebElement logOutIconElement = TribuneDriver.getInstance().getDriver().findElement(By.xpath("(//a[contains(@href, '#')])[10]"));  //LOGS OUT BUTTON
        try {
            Assert.assertNotNull(logOutIconElement);
    ...
    //this is to Sign Up (Register) for an account to make comments on the Comments page
    @FindBy(xpath=" //a[@data-reg-handler='modal-register']")
    private WebElement registerElement;
    @FindBy(css = "html body div#reg-overlay div.reg-dialog.register div.reg-content form button.reg-submit")
    private WebElement registerButton;
    @FindBy(css = "html body div#reg-overlay div.reg-dialog.register div.reg-close")
    private WebElement closePanel;	
    public void signUpForCommentsAccount(String url) throws InterruptedException, IOException {	//11/4 NO SUCH ELEMENT EXCEPTION
        Utils.writeToLog("url to load: " + url);
        goToUrl(url);
        Utils.writeToLog("ENTERING VERIFYCOMMENTSPANEL METHOD->SIGN UP FOR ACCOUNT TO MAKE COMMENTS");
        registerElement.click(); 
        if(registerElement.isDisplayed()) {
            Assert.assertNotNull(registerElement);
            Utils.writeToLog("CORRECT: Register for Comment Page account link is present.");
            Utils.writeToLog("Register link:" + registerElement);
            //this is fill out account registration form for submitting comments
            WebDriver newwindow2 = switchHandleToPopup();
            newwindow2.findElement(By.name("email")).clear();
            newwindow2.findElement(By.name("email")).sendKeys("scottmaretick@yahoo.com");  //email
            newwindow2.findElement(By.name("password")).clear();
            newwindow2.findElement(By.name("password")).sendKeys("sm110751");  //password
            newwindow2.findElement(By.name("zipCode")).clear();
            newwindow2.findElement(By.name("zipCode")).sendKeys("60018");  //zip code
            registerButton.click();
            //account already exists so close panel or ignore
            closePanel.click(); 
            Utils.writeToLog("CORRECT: Registration for account on Comment Page creation is attempted SUCCESSFULLY.");
        }
        
        try {
            registerElement.isDisplayed();
            Assert.assertNotNull(registerElement);
            assert(registerElement).isDisplayed();
            Utils.writeToLog("taking screen shot NGUX131-03");
        } catch (NoSuchElementException e) {
            Utils.writeToLog("registerElement problem");
        }
    }
    //*****************************VERIFY COMMENTS WIDGETS********************************************************
    @FindBy(xpath="//div[@class='vf-comment-textarea']")
    private List<WebElement> CommentsTextArea;	
    public void verifyCommentsCommentsTextArea(String url) throws InterruptedException, IOException {
        Utils.writeToLog("ENTERING VERIFYCOMMENTSPANEL METHOD->VERIFYING COMMENTS AREA IS PRESENT");
        goToUrl(url);
        if(CommentsTextArea.size() > 0) { //WORKS 11/4 1900 COMMENTSTEXTAREASIZE=1
            Utils.writeToLog("CommentsTextArea is there the size is:" + CommentsTextArea.size());
        } else {
            Utils.writeToLog("CommentsTextArea is not there the size is:" + CommentsTextArea.size());
        }
        
        try {
            Assert.assertEquals(CommentsTextArea.size(), 1);
        } catch (NoSuchElementException e) {
            Utils.writeToLog("CommentsTextArea problem");
            
        }
    }
    
    @FindBy(xpath="//div[@class='trb_panelmod_body_content']")
    private List<WebElement> CommentsPanelModBody;	
    public void verifyCommentsPanelModBody(String url) throws InterruptedException, IOException {
        Utils.writeToLog("ENTERING VERIFYCOMMENTSPANEL METHOD->VERIFYING COMMENTS PANELMOD IS PRESENT");
        goToUrl(url);
        if(CommentsPanelModBody.size() > 0) { 	//WORKS 11/4 1900
            Utils.writeToLog("CommentsPanelModBody is there the size is:" + CommentsPanelModBody.size());
        } else {
            Utils.writeToLog("CommentsPanelModBody is not there the size is:" + CommentsPanelModBody.size());
        }
        try {
            Assert.assertEquals(CommentsPanelModBody.size(), 1);	//WORKS 11/4 1900
        } catch (NoSuchElementException e) {
            Utils.writeToLog("CommentsPanelModBody problem");
            
        }
    }
    
    @FindBy(xpath="//div[@class='trb_panelmod_showpener']")
    private List<WebElement> CommentsPanelOpener;	
    public void verifyCommentsPanelOpener(String url) throws InterruptedException, IOException {
        Utils.writeToLog("ENTERING VERIFYCOMMENTSPANEL METHOD->VERIFYING COMMENTS OPENER IS PRESENT");
        goToUrl(url);
        if(CommentsPanelOpener.size() > 0) {	//WORKS 11/4 1900
            Utils.writeToLog("CommentsPanelOpener is there the size is:" + CommentsPanelOpener.size());
        } else {
            Utils.writeToLog("CommentsPanelOpener is not there the size is:" + CommentsPanelOpener.size());
        }
        try {
            Assert.assertEquals(CommentsPanelOpener.size(), 1);	//WORKS 11/4 1900
        } catch (NoSuchElementException e) {
            Utils.writeToLog("CommentsPanelOpener problem");
        }
    }
    
    @FindBy(xpath="//div[@class='trb_panelmod_closer_wedge_row']")
    private List<WebElement> CommentsPanelCloser;	
    public void verifyCommentsPanelCloser(String url) throws InterruptedException, IOException {
        Utils.writeToLog("ENTERING VERIFYCOMMENTSPANEL METHOD->VERIFYING COMMENTS CLOSER IS PRESENT");
        goToUrl(url);
        if(CommentsPanelCloser.size() > 0) {	//WORKS 11/4 1900
            Utils.writeToLog("CommentsPanelCloser is there the size is:" + CommentsPanelCloser.size());
        } else {
            Utils.writeToLog("CommentsPanelCloser is not there the size is:" + CommentsPanelCloser.size());
        }
        try {
            Assert.assertEquals(CommentsPanelCloser.size(), 1);	//NOT SURE 11/4 IF WORKED
        } catch (NoSuchElementException e) {
            Utils.writeToLog("CommentsPanelCloser problem");
        }
    }
    
    
    @FindBy(xpath="//div[@class='vf-child-comments']")
    private List<WebElement> ChildComments;		
    public void verifyChildComments(String url) throws InterruptedException, IOException {
        Utils.writeToLog("ENTERING VERIFYCOMMENTSPANEL METHOD->VERIFYING COMMENTS CHILD COMMENTS PRESENT");
        goToUrl(url);
        if(ChildComments.size() > 0) {		//WORKS 11/4=5 COMMENTS
            Utils.writeToLog("ChildComments is there the size is:" + ChildComments.size());
        } else {
            Utils.writeToLog("ChildComments are not there the size is:" + ChildComments.size());
        }
        try {
            Assert.assertTrue(ChildComments.size() > 0);
            Utils.writeToLog("there are child comments");
        } catch (NoSuchElementException e) {
            Utils.writeToLog("screenshot ChildComments problem");
        }
        
    }
}
//*************************************END COMMENTS NGUX-131 CODE***********************************************************************
//	public void verifyPageNotFoundURL(String url) throws InterruptedException, IOException {
//		Assert.assertEquals("Page Not Found - Chicago Tribune", driver.getTitle());
//		if(getTitle().contains("Page Not Found - Chicago Tribune")) {
//			Utils.logCorrectCheck("Window title " + getTitle());
////			Utils.writeToLog("Window title is:" + getTitle());
//		} else {
//			Utils.logWrongCheck("Window title " + getTitle());
////			Utils.writeToLog("Window title is:" + getTitle());
//		}
//		Utils.writeToLog("calling sleep then getDriver.close");
//
//		try {
//			File scrFile = ((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE);
//			FileUtils.copyFile(scrFile, new File("/Users/smaretick/Documents/workspace/NGUX-MON/test-output/PageNotFound.png"));
//			Utils.writeToLog("taking screen shot");
//		} catch (NoSuchElementException e) {
//			Utils.writeToLog("screenshot problem");
//
//		}
